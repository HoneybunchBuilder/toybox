...
import subprocess
import os
from .cmake import get_out_dir, get_exe, run_build
...
class TbRefreshComponents(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Try to dodge any other errors reported
        json_start_idx = meta.find(b"{")
        meta = meta[json_start_idx:]
        ...
        for comp_name in meta_json:
            ...
            prop_class = type(prop_class_name, (bpy.types.PropertyGroup,), {})

            prop_class.__annotations__["name"] = bpy.props.StringProperty(
                name="name", default=lower_name
            )
            ...
            for meta_name in comp_meta:
                ...
                if meta_type == "bool":
                    prop_class.__annotations__[meta_name] = bpy.props.BoolProperty(
                        name=meta_name, default=False, update=comp_update
                    )
                    ...
                    if "range" in meta_params:
                        ...
                        flt_min = val_range[0]
                        flt_max = val_range[1]
                    prop_class.__annotations__[meta_name] = bpy.props.FloatProperty(
                        name=meta_name,
                        default=0.5,
                        min=flt_min,
                        max=flt_max,
                    ...
                    prop_class.__annotations__[meta_name] = bpy.props.StringProperty(
                        name=meta_name, default="static", update=comp_update
                    )
            ...
            setattr(
                bpy.types.Object,
                prop_name,
                bpy.props.PointerProperty(name=lower_name, type=prop_class),
            )
