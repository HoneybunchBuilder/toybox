...
float3 calc_wave_pos(int3 local_pos, float4x4 m, float4 rot, float3 offset,
                     float time, inout float3 tangent, inout float3 binormal) {
  ...
  float3 pos = qrot(rot, mul(m, float4(local_pos, 1)).xyz) + offset;
